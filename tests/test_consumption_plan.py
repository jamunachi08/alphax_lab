#!/usr/bin/env python3
# Copyright (c) 2026, Neotec Integrated Solutions and contributors
"""Standalone tests for the consumption plan builder.

Runs without a bench or database — frappe is stubbed with fixture data, so this
can go in CI ahead of any site being available.

    python tests/test_consumption_plan.py
"""

import sys
import types
import pathlib

MAPS = {
	"LAB-CBC": {"name": "CBC", "sample_type": "Whole Blood EDTA"},
	"LAB-LFT": {"name": "LFT", "sample_type": "Serum"},
	"LAB-URINE": {"name": "Urine Routine", "sample_type": "Urine"},
	"LAB-EMPTY": {"name": "Mapped But No Consumption Record", "sample_type": "Serum"},
}

# Each test carries its own complete consumable list, draw items included.
CONSUMABLES = {
	"LAB-CBC": [
		{"item": "NEEDLE-21G", "qty": 1},
		{"item": "COTTON", "qty": 2},
		{"item": "TUBE-EDTA", "qty": 1},
		{"item": "REG-CBC", "qty": 1},
		{"item": "CTRL-CBC", "qty": 0.05},
	],
	"LAB-LFT": [
		{"item": "NEEDLE-21G", "qty": 1},
		{"item": "COTTON", "qty": 2},
		{"item": "TUBE-SERUM", "qty": 1},
		{"item": "REG-LFT", "qty": 1},
		{"item": "CUVETTE", "qty": 4},
	],
	"LAB-URINE": [
		{"item": "CUP-URINE", "qty": 1},
		{"item": "DIPSTICK", "qty": 1},
	],
}


class _D(dict):
	__getattr__ = dict.get


def install_stubs():
	f = types.ModuleType("frappe")
	f._dict = _D

	def get_all(dt, filters=None, fields=None, order_by=None, limit=None, pluck=None):
		filters = filters or {}
		if dt == "Plasma Test Map":
			m = MAPS.get(filters.get("item"))
			return [_D(m)] if m else []
		if dt == "Lab Test Consumption":
			item = filters.get("item")
			if isinstance(item, list):
				item = None
			return [item] if item in CONSUMABLES else []
		if dt == "Lab Consumable Item":
			return [_D(r) for r in CONSUMABLES.get(filters.get("parent"), [])]
		return []

	f.get_all = get_all
	f.get_cached_value = lambda dt, name, field: "Nos"
	f.throw = lambda *a, **k: None
	f.msgprint = lambda *a, **k: None
	f.bold = lambda x: x
	f.new_doc = lambda *a: None
	f.get_doc = lambda *a, **k: None
	f.get_cached_doc = lambda *a: None
	f.get_single = lambda *a: None
	f.log_error = lambda *a, **k: None
	f.whitelist = lambda *a, **k: (lambda fn: fn)

	class DNE(Exception):
		pass

	f.DoesNotExistError = DNE
	f.db = types.SimpleNamespace(sql=lambda *a, **k: [], exists=lambda *a, **k: False)
	f.utils = types.SimpleNamespace(get_link_to_form=lambda *a: "")
	f._ = lambda s: s
	sys.modules["frappe"] = f

	u = types.ModuleType("frappe.utils")
	u.cint = int
	u.flt = float
	u.getdate = lambda x=None: x
	u.nowdate = lambda: "2026-08-30"
	sys.modules["frappe.utils"] = u

	sys.modules["frappe.model"] = types.ModuleType("frappe.model")
	mdd = types.ModuleType("frappe.model.document")

	class Document:
		pass

	mdd.Document = Document
	sys.modules["frappe.model.document"] = mdd


class Row:
	def __init__(self, code, qty, group=None):
		self.item_code = code
		self.qty = qty
		self.item_group = group


class Doc(dict):
	def __init__(self, items, is_return=0):
		self._items = items
		self["is_return"] = is_return

	def get(self, key, default=None):
		if key == "items":
			return self._items
		return dict.get(self, key, default)


def main():
	install_stubs()
	sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1]))
	from alphax_lab.lab import consumption

	failures = []

	def check(label, condition):
		print(f"  {'PASS' if condition else 'FAIL'}  {label}")
		if not condition:
			failures.append(label)

	print("single test")
	plan, _ = consumption.build_plan(Doc([Row("LAB-CBC", 1)]))
	check("draw items come from the test itself", plan.get("NEEDLE-21G") == 1)
	check("one EDTA tube", plan.get("TUBE-EDTA") == 1)
	check("fractional control qty preserved", plan.get("CTRL-CBC") == 0.05)

	print("two blood tests on one document")
	plan, _ = consumption.build_plan(Doc([Row("LAB-CBC", 1), Row("LAB-LFT", 1)]))
	check("shared consumables accumulate across tests", plan.get("NEEDLE-21G") == 2)
	check("cotton accumulates", plan.get("COTTON") == 4)
	check("each test's own container", plan.get("TUBE-EDTA") == 1 and plan.get("TUBE-SERUM") == 1)
	check("reagents kept separate", plan.get("REG-CBC") == 1 and plan.get("REG-LFT") == 1)

	print("line qty 2")
	plan, _ = consumption.build_plan(Doc([Row("LAB-CBC", 2)]))
	check("everything scales with line qty", plan.get("REG-CBC") == 2 and plan.get("NEEDLE-21G") == 2)

	print("non-blood test")
	plan, _ = consumption.build_plan(Doc([Row("LAB-URINE", 1)]))
	check("no draw items", "NEEDLE-21G" not in plan)
	check("cup and dipstick issued", plan.get("CUP-URINE") == 1 and plan.get("DIPSTICK") == 1)

	print("unmapped item")
	plan, _ = consumption.build_plan(Doc([Row("SERVICE-CONSULT", 1)]))
	check("empty plan, no consumption", not plan)

	print("unconfigured item detection")
	settings = _D({
		"enabled": 1,
		"unconfigured_item_action": "Block on Submit",
		"lab_item_groups": [_D({"item_group": "Lab Tests"})],
	})
	consumption.get_settings = lambda: settings

	doc = Doc([Row("LAB-CBC", 1, group="Lab Tests")])
	check("configured test passes", not consumption.find_unconfigured(doc))

	doc = Doc([Row("LAB-UNMAPPED", 1, group="Lab Tests")])
	problems = consumption.find_unconfigured(doc)
	check("unmapped item flagged", len(problems) == 1 and problems[0][0] == "LAB-UNMAPPED")

	doc = Doc([Row("LAB-EMPTY", 1, group="Lab Tests")])
	problems = consumption.find_unconfigured(doc)
	check("mapped item with no consumption record flagged", len(problems) == 1)

	doc = Doc([Row("SERVICE-CONSULT", 1, group="Services")])
	check("item outside lab groups ignored", not consumption.find_unconfigured(doc))

	print("credit note")
	plan, _ = consumption.build_plan(Doc([Row("LAB-CBC", -1)], is_return=1))
	check("return reverses the same plan", plan.get("NEEDLE-21G") == 1)

	print()
	if failures:
		print(f"FAILED: {len(failures)}")
		return 1
	print("All checks passed")
	return 0


if __name__ == "__main__":
	sys.exit(main())
