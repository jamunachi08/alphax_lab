...
def validate(doc, method=None):
	"""Reject a lab service line that is not configured to consume anything."""
	settings = get_settings()
	...
	for row in doc.get("items", []):
		...
		if not _get_map(row.item_code):
			unmapped.append(row.item_code)

	if unmapped:
		frappe.throw(
			_("No active Plasma Test Map for: {0}. These lines would consume no stock.").format(
