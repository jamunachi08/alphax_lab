...
Each test has one Lab Test Consumption record listing everything it consumes.
Consumption is the sum of every mapped line's consumables, multiplied by that
line's qty. There are no shared or document-level tiers: what a test consumes is
defined once, in one place, and that place has its own list view.
...
def find_unconfigured(doc):
	...
	for row in doc.get("items", []):
		...
		if not _get_map(row.item_code):
			...
			problems.append((row.item_code, _("no active Lab Test Consumption record")))

	return problems


def _has_consumables(item_code):
	definition = _consumption_doc(item_code)
	if not definition:
		return False
	return bool(
		frappe.get_all(
			"Lab Consumable Item",
			filters={"parent": definition, "parenttype": "Lab Test Consumption"},
			limit=1,
		)
	)
...
def _unconfigured_message(problems):
	...
	return _(
		...
		"Fix either way:<br>"
		"&bull; If it is a real test, add a Plasma Test Map row and a Lab Test Consumption record.<br>"
...
def _add_consumables(plan, test_item, multiplier=1):
	definition = _consumption_doc(test_item)
	if not definition:
		return
...
def _consumption_doc(test_item):
	"""Name of the active Lab Test Consumption record for this item, if any."""
	rows = frappe.get_all(
		"Lab Test Consumption",
		filters={"item": test_item, "is_active": 1},
		pluck="name",
		limit=1,
	)
...
def create_consumption_entry(doc, plan, context, settings):
	...
	for item_code, qty in plan.items():
		...
		if shortfall > flt(settings.shortage_tolerance_qty):
			shortages.append(
				batch_selection.describe_shortage(
					item_code, warehouse, qty, shortfall, posting_date=doc.posting_date
				)
			)
