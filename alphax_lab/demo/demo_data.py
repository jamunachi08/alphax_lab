...
def setup():
	naming_by = frappe.db.get_default("item_naming_by")
	forced = False
	if naming_by == "Naming Series":
		# Item.autoname overwrites item_code with the series when naming is by
		# series, so every DEMO- code would silently become ITEM-0001 and all
		# later lookups would miss.
		frappe.db.set_default("item_naming_by", "Item Code")
		forced = True
		print("Item naming was set to Naming Series; temporarily switched to Item Code.")
	...
	try:
		...
		if forced:
			...
			print("Item naming restored to Naming Series.")
...
def _setup():
	...
	missing = [
		PREFIX + code for code, *_rest in CONSUMABLES if not frappe.db.exists("Item", PREFIX + code)
	] + [PREFIX + t["code"] for t in TESTS if not frappe.db.exists("Item", PREFIX + t["code"])]

	if missing:
		print("\nWARNING: these items were not created under the expected code:")
		for code in missing:
			print(f"  {code}")
		print("Check Stock Settings > Item Naming By.")
		return
...
def _customer():
	...
	groups = frappe.get_all("Customer Group", {"is_group": 0}, pluck="name", limit=1)
	territories = frappe.get_all("Territory", {"is_group": 0}, pluck="name", limit=1)
	if not groups or not territories:
		frappe.throw("No non-group Customer Group or Territory exists. Create one first.")

	doc = frappe.new_doc("Customer")
	doc.customer_name = name
	doc.customer_group = groups[0]
	doc.territory = territories[0]
	doc.flags.ignore_permissions = True
	doc.insert()
	return doc.name
