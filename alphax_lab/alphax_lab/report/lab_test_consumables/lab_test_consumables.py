...
Consumables are defined one record per test in Lab Test Consumption. That list
view shows the records; this report shows what is inside them, so you can compare
ten tests without opening ten records. Use it to spot a test that lists two needles by mistake,
...
def execute(filters=None):
	...
	definitions = frappe.get_all(
		...
		filters={"item": ["in", [t.name for t in tests]], "is_active": 1},
		fields=["name", "item"],
	)
	definition_item = {d.name: d.item for d in definitions}

	rows = frappe.get_all(
		"Lab Consumable Item",
		filters={"parent": ["in", list(definition_item)], "parenttype": "Lab Test Consumption"},
		fields=["parent", "item", "qty", "idx"],
		order_by="parent asc, idx asc",
	) if definition_item else []

	by_test = {}
	for row in rows:
		by_test.setdefault(definition_item[row.parent], []).append(row)
			...
			data.append(
				{
					...
					"stock_on_hand": None,
					"indent": 0,
					"note": _("No active Lab Test Consumption record"),
				}
			)
			continue
