...
See **INSTALL.md** for the full sequence including deploy verification.
...
### Starter catalog without shell access

`import_files/` holds Data Import CSVs for 12 tests, 32 consumables and 28
batches — no bench access needed. See `import_files/README.md` for the order and
the two files that need a find-and-replace first.
...
fields, seed six standard sample types, initialise settings, and add the Lab
workspace. Each step commits separately and retries on lock contention, so a
partial run can be finished by simply running migrate again.

### If install deadlocks

```
(1213, 'Deadlock found when trying to get lock; try restarting transaction')
```

`plasma_ref` is created with a unique constraint on Sales Invoice, Delivery Note
and Stock Entry. On a site with existing rows that means an ALTER TABLE, which
can deadlock against live writes from the scheduler or background jobs.

The constraint is deliberate: it is what makes a retried Plasma webhook safe. An
application-level duplicate check races between two concurrent webhooks; a unique
index does not.

Re-running usually succeeds. To make it reliable on a busy site:
...
To see what landed before a failure:
...
**Batch storage.** v15 keeps batch quantities either on
`Stock Ledger Entry.batch_no` or inside Serial and Batch Bundle entries,
depending on how the stock was received. Availability is summed across both. If
a shortage looks wrong against Stock Balance, run:
...
It prints what the allocator sees per storage path and per batch.
...
python tests/test_batch_selection.py    # batch allocation across both storage paths
python verify_tree.py                   # structural guard, run before commit
```

`verify_tree.py` checks file structure, that `patches.txt` carries both section
headers and lists every patch on disk, doctype module ownership, hook wiring,
